...
require([
  ...
  $(function() {
    ...
    $("#btnSearch").on("click", function() {
      ...
      var _url =
        ...
        _address +
        "&key=AIzaSyDvFT3eOvji9SXf6ckH0ODYXup6E2pprNw";
    ...
    var fl = new FeatureLayer({
      url:
        "http://services3.arcgis.com/1iEaN7ShrrAnHGzH/arcgis/rest/services/Real_Estate_Case/FeatureServer/0",
    ...
    var fl2 = new FeatureLayer({
      url:
        "http://services3.arcgis.com/1iEaN7ShrrAnHGzH/arcgis/rest/services/Real_Estate_Case/FeatureServer/1",
  ...
  function queryFL() {
    ...
    if (layer) {
      //如果找到就建立一個查詢，條件是用geometry來交集(intersect)
  ...
  function displayResults(results) {
    ...
    try {
      ...
      var _avgPrice =
        Math.round((_price / results.features.length / 10000) * 100) / 100;
      var BuildingAge = dt.getFullYear() - Math.round(_avg); //Math.round是四捨五入 getFullyear是取的現在年分
      $("#statistic").css("display", "block");
      $("#avgYear")
        .empty()
        .append("平均屋齡</br>" + BuildingAge);
      $("#avgPrice")
        .empty()
        .append("平均房價(萬元)</br>" + _avgPrice);
    } catch (e) {
      console.log(e);
    }
    //以上為統計buffer的平均屋齡
